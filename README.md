...
O pipeline foi organizado em **seis notebooks sequenciais**, um por responsabilidade. A alternativa — um notebook único — dificultaria reexecutar apenas uma etapa e tornaria a leitura do processo mais confusa.
...
| Seis colunas de valor de texto para `double` | Permitir agregação. Resolve também a notação `0E-8` |
| `trim` nas colunas de texto | Eliminar espaços acidentais que quebrariam agrupamentos |
| Criação de `flag_agregado` | Identificar o registro SIN sem removê-lo |
| Fixação do fuso da sessão em UTC | Garantir que a hora lida seja idêntica à hora do arquivo |

A fixação do fuso merece destaque. O campo `din_instante` chega sem indicação de fuso e o ONS publica em horário de Brasília. Sem fixar o fuso da sessão, o Spark interpreta a data conforme a configuração do ambiente, e o resultado da análise horária mudaria conforme quem executa o notebook. Como a Pergunta 4 depende precisamente da hora do dia, um deslocamento de três horas inverteria a conclusão sobre o descasamento entre carga e geração solar.
...
| Geração ausente no registro | Comparação de cada fonte com os dias 12 a 16/09 | Descartada: eólica, fotovoltaica, hidráulica e térmica todas em valores normais |
...
A verificação de balanço foi convertida em **regra permanente do pipeline**. Em vez de verificar uma única vez, o notebook compara o número de violações ao limite conhecido e interrompe a execução caso surja uma anomalia nova:

```python
LIMITE_CONHECIDO = 24  # Sul, 14/09/2022 — anomalia documentada na origem

violacoes = spark.table("workspace.gold.qa_violacoes_balanco").count()

if violacoes > LIMITE_CONHECIDO:
    raise ValueError(
        f"Balanço energético: {violacoes} violações, acima das {LIMITE_CONHECIDO} "
        f"conhecidas. Nova anomalia detectada — investigar antes de publicar."
    )
```

A diferença prática é relevante: uma verificação pontual informa o estado atual, um contrato de dados impede que uma carga futura publique números incorretos de forma silenciosa.
...
Uma verificação adicional reforça a consistência do resultado: a soma dos quatro subsistemas em cada ano aproxima-se de zero (−75 MWmed em 2019, +347 MWmed em 2025), como esperado de um sistema fechado. O resíduo remanescente corresponde às perdas de transmissão.
...
|---|---|---|---|---|
| 2019 | 14,1 | 1,8 | 106,5 | 27,1 |
| 2022 | 20,9 | 4,5 | 109,5 | 30,5 |
| 2025 | 32,6 | 16,7 | 109,6 | 34,4 |

A eólica mais que dobrou, a solar cresceu nove vezes e a hidráulica estagnou — a mesma história contada pela análise de geração, agora confirmada por um **dataset independente**.
...
| 06 | Violações do balanço energético | 24 (contrato de dados aprovado) |
...
Qualquer divergência nesses valores indica falha no pipeline ou alteração nos dados publicados pela fonte. O contrato de dados do notebook 06 interrompe a execução automaticamente se surgirem violações além das 24 conhecidas.
